...
- 🧾 **成绩单 PDF 补录** — 个别学生缺成绩时，上传教务处正式成绩单 PDF（可多份），自动识别学号/班级与课程明细；按「学号+学年+学期+课程名」与已有成绩匹配，缺失的补录、已有的更正；学号不在系统时自动建档（可单个或批量从已有班级中选择目标班级，或按成绩单班级/学院/专业新建），逐条勾选后才入库，同样支持批次回滚
